...
-- Small module to toggle DAP UI components
local dapui_toggle = {
    visible = false,
    open_win_ids = {},
    layout = function(self)
        ...
        for id = #layouts, 1, -1 do
            if self.visible and layouts[id]:is_open() then
                self.open_win_ids[id] = true
                dapui.close { layout = id }
            elseif not self.visible and self.open_win_ids[id] then
                self.open_win_ids[id] = false
                dapui.open { layout = id, reset = true }
            end
        end
        self.visible = not self.visible
    end,
    element = function(self, id)
        local dapui = require('dapui')
        return function()
...
local nvim_dap_ui = {
    ...
    config = function(_, opts)
        ...
        dap.listeners.after.event_initialized.dapui_init_layout = function()
            ...
            dapui.open { layout = 1 }
            dapui.open { layout = 4 }
            nnoremap('<Leader>dut', function() dapui_toggle:layout() end)
            nnoremap('<Leader>duv', dapui_toggle:element(1)) -- Variables
            nnoremap('<Leader>dur', dapui_toggle:element(2)) -- REPL
            nnoremap('<Leader>dub', dapui_toggle:element(3)) -- Breakpoints
            nnoremap('<Leader>duc', dapui_toggle:element(4)) -- Console
        end
