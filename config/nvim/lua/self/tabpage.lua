    ...
    vim.ui.input(
      {
        ...
        highlight = function(name)
          ...
          if #name > MAX_NAME_LEN then return { { MAX_NAME_LEN, #name, 'ErrorMsg' } } end
          -- Error highlight for conflict with existing names
          if existing_names[name] then return { { 0, #name, 'ErrorMsg' } } end
          return {}
        end,
        prompt = curr_name and ' Rename tabpage: ' or ' New tabpage name: ',
      ...
      function(name)
        ...
        elseif #name > MAX_NAME_LEN then -- Check: name too long
